...
static ssize_t read_line(int fd, char *buffer, size_t size) { // VULNERABILITY: not null terminated
...
static bool sanitize_path(char *buffer) { // don't allow "../"
...
static bool list_songs() {
  ...
  char path[48] = "./data/";// VULNERABILITY!
...
int main() {
  ...
  while (1) {
    ...
    printf("Command> ");
    read_line(STDIN_FILENO, buffer, sizeof(buffer));// not end

    // cwd is .
    if (!strcmp(buffer, "bands")) {         // cmd -> ls ./data
      if (!list_bands()) {
        break;
      }
    } else if (!strcmp(buffer, "songs")) {  // traversal vulnerability
      if (!list_songs()) {
        break;
      }
